...
version to force compilation of libpolymake against libcxxwrap-julia. The release has been somewhat hastened by the need for for `Oscar.jl` jointly working with the newest `CxxWrap.jl`, so expect rather `v.0.4.1` and later minor versions rather heavy in added features :)
