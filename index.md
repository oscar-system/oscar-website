...
<div style="background: #f4f4f4;
  ...
  Hans Schönemann passed away on {{ "2025-12-28" | date: site.data.config.date_format }}.
...
{% if upcoming_conferences.size > 0 %}
  {% for event in upcoming_conferences limit:max_events %}
    {% if event.website %}
  * [{{ event.title }} ({{ event.location }}, {{ event.start-date | date: site.data.config.date_format }} to {{ event.end-date | date: site.data.config.date_format }})]({{ event.website | replace: "https://www.oscar-system.org", site.baseurl }})
    {% else %}
  * {{ event.title }} ({{ event.location }}, {{ event.start-date | date: site.data.config.date_format }} to {{ event.end-date | date: site.data.config.date_format }})
    {% endif %}
  {% endfor %}
...
_Released on {{ site.data.release.date | date: site.data.config.date_format }}_ -- [View release notes.](https://github.com/oscar-system/Oscar.jl/releases/tag/v{{ site.data.release.version }})
