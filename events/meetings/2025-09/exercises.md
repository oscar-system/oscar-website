...
Hint: (in AbstractAlgebra: df_df, _interpolation, _clow, _fflue, _popov
    Nemo: det_given_divisor
    Hecke
    Oscar)

Can you find refernces/ explanations for the algorithms implemented?
...
Decide if 2 number fields are linearly disjoint, ie. the spltting fields are
disjoint.
...
- Find the following numerical invariants: dimension, degree, geometric genus.
...
Recall the "weak" Nullstellenssatz: a polynomial system $S = \{f_1,...,f_r\}$
...
gives a distinct colouring of $G$.  By the Nullstellenssatz there is no
...
the ideal generated by the polynomials contains 1 (think of the "Weak Nullstellenssatz").
You now have enough information to start searching!

<details>Hint: the condition "$X$ must be non-zero" can be expressed by the polynomial
