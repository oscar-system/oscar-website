---
layout: meeting
title: Participants
meeting: true
meeting_nr: 202602
people:
    - {name: John Abbott, affiliation: RPTU Kaiserslautern-Landau}
    
    - {name: Thomas Breuer, affiliation: RWTH Aachen}
    
    - {name: Shelby Cox, affiliation: MPI MiS Leipzig}
    
    - {name: Antony Della Vecchia, affiliation: TU Berlin}

    - {name: Kamillo Ferry, affiliation: TU Berlin}
    
    - {name: Claus Fieker, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Zoe Geiselmann, affiliation: TU Berlin}

    - {name: Lars Göttgens, affiliation: RWTH Aachen}

    - {name: Friedemann Groh, affiliation: ISG Stuttgart}

    - {name: Jeroen Hanselman, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Max Horn, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Michael Joswig, affiliation: TU Berlin}

    - {name: Lars Kastner, affiliation: TU Berlin}

    - {name: Kevin Kühn, affiliation: TU Berlin}

    - {name: Fabian Lenzen, affiliation: TU Berlin}

    - {name: Benjamin Lorenz, affiliation: TU Berlin}

    - {name: Igor Makhlin, affiliation: TU Berlin}

    - {name: Stef Nomden, affiliation: Leiden University}

    - {name: Yue Ren, affiliation: Durham University}

    - {name: Emre Sertöz, affiliation: Leiden University}

    - {name: Marcel Wack, affiliation: TU Berlin}

    - {name: Martin Wagner, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Matthias Zach, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Roan Talbut, affiliation: Durham University}

    - {name: Oliver Clarke, affiliation: Durham University}

    - {name: Gabriel Riffo Jara, affiliation: TU Berlin}

    - {name: Aaruni Kaushik, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Dante Luber, affiliation: Paderborn University}

    - {name: Victoria Schleis, affiliation: Durham University}

    - {name: Madhavan Venkatesh, affiliation: MPI SWS, Saarbrücken}

    - {name: Gabriel Ong, affiliation: University of Bonn}

    - {name: Christian Eder, affiliation: RPTU Kaiserslautern-Landau}

    - {name: Leonard Schmitz, affiliation: TU Berlin}
    
    
---

{% include participants.html participants=page.people %}

<!-- [<img src="../groupphoto.jpg" />](../groupphoto.jpg) -->
